...
The turntable's power outlet is a Matter smart plug commissioned onto BabelPod's own fabric (multi-admin — it stays paired with Apple Home). Configured via `turntablePlugEnabled`/`turntablePlugPairingCode` in `babelpod.config.json`; credentials persist in `.matter-storage/`. The OnOff attribute subscription drives every `turntablePower` broadcast (never synthesized from commands), so clients always see real hardware state. Silence auto-off (`autoOffEnabled`, threshold/noise floor in dBFS, duration in minutes) taps the same RMS pipeline as autoconnect and cuts the plug after sustained signal in the dead band between the noise floor and the threshold (a powered turntable with nothing playing); a line below the noise floor means the turntable is already off and the plug is left alone; it is a privileged automation path that bypasses the session-owner lock without changing ownership. See `API.md` § "Turntable Power & Silence Auto-Off".
