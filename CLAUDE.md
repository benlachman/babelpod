...
The server runs on a **Raspberry Pi Zero 2 W** (ARM64, 512MB RAM, Debian/Trixie). Audio capture and playback use ALSA tools (`arecord`, `aplay`) via spawned child processes. AirPlay streaming uses `node_airtunes2` — pinned to a tag of our fork (`benlachman/node_airtunes2#v2.4.9-babelpod.2`, branch `babelpod`, from `ciderapp`'s `v2.4.9`). The fork adds a `name` option to `add()` so the AirPlay receiver prompt shows the server's display name, and replaces `fast-srp-hap`'s pure-JS SRP with a native-BigInt client (`lib/hap_srp.js`) so AirPlay 2 pairing takes ~0.3 s per receiver on the Pi instead of ~2.5 s. To change the fork: commit on its `babelpod` branch, cut a new `v2.4.9-babelpod.N` tag, bump the pin here, and run `npm install` on the Pi after deploying; BabelPod passes `config.displayName`. Receivers advertising AirPlay 2 transient pairing (`features` bit 48 — HomePods, Apple TVs) are streamed over node_airtunes2's AirPlay 2 path (`airplay2: true` + the mDNS TXT record); others use legacy RAOP. This is mandatory since HomePod software / tvOS 27, which silently drop RAOP audio (see `airplay2Enabled` in API.md). Bluetooth input uses `bluetoothctl` via a Node wrapper. Device discovery uses `dnssd2` (mDNS) and `mdns-js`.
...
The turntable's power outlet is a Matter smart plug commissioned onto BabelPod's own fabric (multi-admin — it stays paired with Apple Home). Commissioned in-app from a pairing code via the `setupTurntablePlug` event (runtime, no restart — `MatterPlugController.init()`/`commission()`/`connect()`), or the legacy file-based path (`turntablePlugEnabled`/`turntablePlugPairingCode` in `babelpod.config.json`). Credentials persist in `.matter-storage/`; the pairing code is never broadcast (stripped from `config`/`state` via `publicConfig()`). Single plug only. The OnOff attribute subscription drives every `turntablePower` broadcast (never synthesized from commands), so clients always see real hardware state. Silence auto-off (`autoOffEnabled`, threshold/noise floor in dBFS, duration in minutes) taps the same RMS pipeline as autoconnect and cuts the plug after sustained signal in the dead band between the noise floor and the threshold (a powered turntable with nothing playing); a line below the noise floor means the turntable is already off and the plug is left alone; it is a privileged automation path that bypasses the session-owner lock without changing ownership. See `API.md` § "Turntable Power & Silence Auto-Off".
